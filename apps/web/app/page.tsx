...
import { MajorEarthquakeBanner } from "../components/major-earthquake-banner";
import { PeruMap } from "../components/peru-map";
import { ProviderSwitcher } from "../components/provider-switcher";
import { MAJOR_EARTHQUAKE_THRESHOLD } from "../lib/emergency";
...
export async function CountryHomePage({
  ...
  let majorEvent: Awaited<ReturnType<typeof getRecentMajorEvent>> = null;
  let loadError: unknown = null;
  const [latestResult, majorEventResult] = await Promise.allSettled([
    getLatestEvent(provider),
    getRecentMajorEvent(provider, MAJOR_EARTHQUAKE_THRESHOLD),
  ]);
  if (latestResult.status === "fulfilled") latest = latestResult.value;
  else loadError = latestResult.reason;
  if (majorEventResult.status === "fulfilled") {
    majorEvent = majorEventResult.value;
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-5">
      ...
      {majorEvent ? (
        <MajorEarthquakeBanner event={majorEvent} country={country} />
      ) : null}
