...
| V4 API y CLI para eventos | DONE | `docs/evidence/v3-v5/api-explorer-executed.png`, `cli-events-table.txt`; OpenAPI 3.1 en `/api/v1/openapi.json`; contract tests con respuestas reales capturadas | — |
...
| VC1 Provider Colombia | DONE, FLAG | `/colombia`, `/colombia/sismos` con año en curso y M≥3, API `provider=sgc&since=ytd`, CLI `--provider sgc --since ytd`, mapa ISO 170 y detalle `sgc-SGC...` | Producción requiere `SISMO_SGC_PROVIDER=true`; estaciones y ondas aún son exclusivas del IGP |
| VC2 Emergencia versionada | CODE-COMPLETE | `/colombia/emergencia`, `/v1/incidents/{slug}`, `sismo incident`, Neon, Trigger.dev cada minuto y revisión humanitaria privada | La migración Neon debe aprobarse y aplicarse antes de habilitar persistencia en producción |
